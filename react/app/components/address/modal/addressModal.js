...
const reducer = (state, action) => {
    if (action != '') {
        return ({ ...state, name: action[0].name })
    }
}
const initialState = {
    name: ''
...
function addressLeft(props) {
    ...
    const [add, setAdd] = useState(props.Id)
    const [name1, dispatch] = useReducer(reducer, initialState)

    //edit
    useEffect(() => {
        axios
            .post(`http://localhost:5000/address/GetAddress`, { ...add })
            .then(res => {
                return dispatch(res.data.data)
            })
            .catch(err => {
                console.log(err)
            })
